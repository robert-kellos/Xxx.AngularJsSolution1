(function () {
    ...
    angular
        .module('app')
        .controller('createeOrderController', createeOrderCtrl);

    createeOrderCtrl.$inject = ['_', '$location', '$state', '$scope', '$filter', 'eordersService', 'eorderUtils', '$timeout', '$q', '$log', '$mdDialog'];

    function createeOrderCtrl(_, $location, $state, $scope, $filter, eordersService, orderUtils, $timeout, $q, $log, $mdDialog) {
        ...
        vm.initNewOrder = function ($event) {
            ...
            if ($state.$current.name === 'createeorder') {
                $state.reload();
            } else {
                $state.go('createeorder')
            }
        }
